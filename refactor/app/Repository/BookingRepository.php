...
class BookingRepository extends BaseRepository
{
    ...
    const TRANSLATOR_TYPE_PROFESSIONAL  = 'professional';
    const TRANSLATOR_TYPE_RWSTRANSLATOR = 'rwstranslator';
    const TRANSLATOR_TYPE_VOLUNTEER     = 'volunteer';

    const SPECIFIC_JOB                  = 'SpecificJob';
    const USER_CANNOT_ACCEPT_JOB        = 'userCanNotAcceptJob';
    ...
    public function store($user, $data)
    {
        ...
        if ($user->user_type == env('CUSTOMER_ROLE_ID')) {
            ...
            /** checking if any field is missing or not and returing the fail response */
    ...
    public function getPotentialJobIdsWithUserId($user_id)
    {
        ...
        $job_type = 'unpaid';
        if ($translator_type == self::TRANSLATOR_TYPE_PROFESSIONAL)
            $job_type = 'paid';   /*show all jobs for professionals.*/
        else if ($translator_type == self::TRANSLATOR_TYPE_RWSTRANSLATOR)
            $job_type = 'rws';  /* for rwstranslator only show rws jobs. */
        else if ($translator_type == self::TRANSLATOR_TYPE_VOLUNTEER)
            ...
            if ($oneUser->user_type == '2' && $oneUser->status == '1' && $oneUser->id != $exclude_user_id) { // user is translator and he is not disabled
                ...
                foreach ($jobs as $oneJob) {
                    if ($job->id == $oneJob->id) { // one potential job is the same with current job
                        ...
                        if ($job_for_translator == self::SPECIFIC_JOB) {
                            $job_checker = Job::checkParticularJob($userId, $oneJob);
                            if (($job_checker != self::USER_CANNOT_ACCEPT_JOB)) {
    ...
    public function getPotentialTranslators(Job $job)
    {
        ...
        if ($job_type == 'paid')
            $translator_type = self::TRANSLATOR_TYPE_PROFESSIONAL;
        else if ($job_type == 'rws')
            $translator_type = self::TRANSLATOR_TYPE_RWSTRANSLATOR;
        else if ($job_type == 'unpaid')
            $translator_type = self::TRANSLATOR_TYPE_VOLUNTEER;
    ...
    public function getPotentialJobs($cuser)
    {
        ...
        foreach ($job_ids as $k => $job) {
            ...
            if ($job->specific_job == self::SPECIFIC_JOB)
                if ($job->check_particular_job == self::USER_CANNOT_ACCEPT_JOB)
                    unset($job_ids[$k]);
            ...
            if (isset($requestdata['filter_timetype']) && in_array($requestdata['filter_timetype'], ['created', 'due'])) {
                if (isset($requestdata['from']) && !empty($requestdata['from'])) {
                    ...
                    if ($requestdata['filter_timetype'] === 'due') {
                        $allJobs->where('jobs.due', '>=', $from);
                    } else {
                        $allJobs->where('jobs.created_at', '>=', $from);
                    }
                }
